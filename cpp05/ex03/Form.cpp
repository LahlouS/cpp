...
Form::Form() : _name("02345904582fff12"), _target("default"), _isSign(0), _grade_exec(150), _grade_sign(150){
	std::cout << "[Form] Default construcor has been called\n";
}


Form::Form(std::string const & name, std::string const & target, int grade_exec, int grade_sign)\
: _name(name), _target(target),_isSign(0), _grade_exec(grade_exec), _grade_sign(grade_sign){
...
void	Form::execute(Bureaucrat const & executor) const{
	if (executor.getGrade() <= this->_grade_exec && _isSign)
		doTheJob(executor);
