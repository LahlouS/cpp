...
ClapTrap::ClapTrap() : _name("Default"), _hitPoint(100), _energyPoints(50), _dammage(20) {
	//std::cout << "Default construcot has been called" << std::endl;
}

ClapTrap::ClapTrap(std::string const & name) : _name(name), _hitPoint(100), _energyPoints(50), _dammage(20) {
	std::cout << "[" << _name << "]" << "Assignement constructor has been called" << std::endl;
}
